...
n_train = n_maxtrain
x_train, y_train, scaler = trainingdata(pcs, n_pc, n_train, 'standard_top_pc')
...
np.save('RMSE_Psi1_100days_predictions_LSTM+Noise_npc=150_nic=9990_nensem=100_ntrain=400K_ntest=100K',meanRMSE)
...
#plt.savefig('RMSE_Psi1_50days_predictions_LSTM+Noise_npc=150_nic=9995_nensem=100_ntrain=400K_ntest=100K.png',dpi=100)
#plt.close()
...
plt.colorbar()
plt.clim([0, 1])
plt.savefig('ITCC_Psi1_100days_predictions_LSTM+Noise_npc=150_nic=9990_nensem=100_ntrain=400K_ntest=100K.png',dpi=100)
plt.close()
np.save('ITCC_Psi1_100days_predictions_LSTM+Noise_npc=150_nic=9990_nensem=100_ntrain=400K_ntest=100K',meanitcc)
...
#plt.ylim([0.7, 1])
#plt.grid(color='k', linestyle='--', linewidth=0.2)
#plt.savefig('ISCC_Psi1_50days_predictions_LSTM+Noise_npc=150_nic=9995_ntrain=400K_ntest=100K.png',dpi=100)
#plt.close()
np.save('ISCC_Psi1_100days_predictions_LSTM+Noise_npc=150_nic=9990_nensem=100_ntrain=400K_ntest=100K',meaniscc)
"""
################################################################
#################### Long term forecasts #######################
################################################################
n_maxlead = 40000  # Prediction of 1000 records for each realization
...
plt.clim(-20, 20)
plt.colorbar()
plt.savefig('Climatology_Psi1_400Kdays_predictions_LSTM+Noise_npc=150_nic=1_nensem=1_ntrain=400K_ntest=100K.png',dpi=100)
plt.close()

plt.imshow(mean_psi1_variance_pred.reshape(nx,ny), origin='lower', cmap='jet')
plt.clim(0, 100000)
plt.colorbar()
plt.savefig('Variance_Psi1_400Kdays_predictions_LSTM+Noise_npc=150_nic=1_nensem=1_ntrain=400K_ntest=100K.png',dpi=100)
plt.close()

plt.imshow(np.array(mean_freq_pred).reshape(nx,ny), origin='lower', cmap='jet')
plt.colorbar()
plt.clim([0,4])
plt.savefig('Frequency_Psi1_400Kdays_predictions_LSTM+Noise_npc=150_nic=1_nensem=1_ntrain=400K_ntest=100K.png',dpi=100)
plt.close()
