...
################################################################
#### Load the predictions #####
f = 'psi1_pred_150PCs_tlength=400K_lead=200K_icond=1_inorm=0_nensembles=1.dat'
n_maxlead = 20000
n_ic = 1
y_pred = dlmread(f,n_ic*n_maxlead,n_pc,',') 
...
plt.clim(-20, 20)
plt.colorbar()
plt.savefig('Climatology_Psi1_200Kdays_predictions_MLLR_npc=150_norm=topPC_nic=1_nensem=1_ntrain=400K.png',dpi=100)
plt.close()

plt.imshow(mean_psi1_variance_pred.reshape(nx,ny), origin='lower', cmap='jet')
plt.clim(0, 100000)
plt.colorbar()
plt.savefig('Variance_Psi1_200Kdays_predictions_MLLR_npc=150_norm=topPC_nic=1_nensem=1_ntrain=400K.png',dpi=100)
plt.close()

plt.imshow(np.array(mean_freq_pred).reshape(nx,ny), origin='lower', cmap='jet')
plt.colorbar()
plt.clim([0,4])
plt.savefig('Frequency_Psi1_200Kdays_predictions_MLLR_npc=150_norm=topPC_nic=1_nensem=1_ntrain=400K.png',dpi=100)
plt.close()
"""
